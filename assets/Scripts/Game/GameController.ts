import { _decorator, CCInteger, Collider2D, Component, Contact2DType, Input, input, IPhysics2DContact, math, Node, Prefab, Quat, RigidBody2D, Vec2, Vec3,Animation, cclegacy, log, director } from 'cc';
...
export class GameController extends Component {
    ...
    protected update(deltaTime: number): void {
        if(this.checkStart&&this.checkStart1)
        {
            ...
            if (this.checkFly) {
                this.gameModel.Character.position = new Vec3(this.gameModel.Character.position.x, this.gameModel.Character.position.y + 12);
        ...
        for (let i = 0; i < numEnemies; i++) {
            ...
            Enemy && (Enemy.GetNode().position=new Vec3(Enemy.GetNode().position.x - this.speed, Enemy.GetNode().position.y));
            ...
            // // Enemy && (Enemy.GetNode().position = new Vec3(i * spacing, 0, 0));
            // Enemy && (Enemy.GetNode().setRotationFromEuler(0, 0, RotationRandom));
            // Enemy && (Enemy.GetNode().position=new Vec3(Enemy.GetNode().position.x - 10, Enemy.GetNode().position.y));
        }
    }
    ...
    private skipRed():void
    {
        ...
        this.gameModel.CharacterAniHead.play('SkipHead');
